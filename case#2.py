                    ...
                    if d4[j] < mini:
                        ...
                        if len(d6[j]) > 1:
                            ...
                            for f in range(1, len(d6[j])):
                                ...
                                time1 = d2[r]['time_to_go'] + count_ozhid - 1
                                d2[d6[j][f]]['time_to_go'] = time1
                                print(time1)
                         # Время отъезда запоминаю, тут можно как раз и сделать доп условие по времени, которое у тебя в тетрадке. ******************ПОДУМАТЬ*******************
                        its_time_to_go[i] = time1 # Ну и это словарь который дальше используем
                        break
